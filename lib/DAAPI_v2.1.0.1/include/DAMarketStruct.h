...
namespace DAMARKET {
	...
	struct CMarketReqUserLoginField
	{
		...
		TDAStringType		UserType;						// 用户类型(保留，未使用)
		TDAStringType		MacAddress;						// MAC地址(保留，未使用)
		TDAStringType		ComputerName;					// 计算机名(保留，未使用)
		TDAStringType		SoftwareName;					// 软件名称(保留，未使用)
		TDAStringType		SoftwareVersion;				// 软件版本号(保留，未使用)
		TDAStringType		AuthorCode;						// 授权码(保留，未使用)
	...
	struct CMarketReqMarketDataField
	{
		...
		TDACharType			SubscMode;						// 请求类型
		TDAStringType		ExchangeCode;					// 交易所
		TDAIntType			MarketCount;					// 单次最大订阅数	
		TDAStringType		MarketTrcode[MAX_SUB_COUNT];	// 单次订阅合约数组		
		TDAStringType		ErrorDescription;				// 错误信息
	};
	...
	// 返回行情数据
	struct CMarketRtnMarketDataField
	{
	...
	// 返回经济商数据
	struct CMarketRtnBrokerDataField
	{
		TDABrokerType		BrokerData;						// 经济商数据
	};

	// 推送交易日结构体
	struct CMarketRtnTradeDateField
	{
